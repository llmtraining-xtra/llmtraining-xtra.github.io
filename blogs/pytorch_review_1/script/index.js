...
async function _load_code_segment(){
    ...
    function __process_language_name(p_language){
        ...
        else if(p_language == 'bash') return 'Bash'
        else if(p_language == 'cpp' || p_language == 'c++') return 'C++'
        else if(p_language == 'c') return 'C'
        else if(p_language == 'go' || p_language == 'golang') return 'Golang'
        else return p_language
    }

    let code_segment_index_list = new Array()
    ...
    for(let i = 0; i < code_segments.length; i++){
        ...
        if(code_segment_meta.getAttribute("class") === "code_segment"){
            ...
            if(code_segment_meta.getAttribute("label")){
                code_segment_index_list[code_segment_meta.getAttribute("label")] = i+1;
            }
        }
        ...
        // 创建左边的固定 block
        let code_segment_btn_left_absolute_block = document.createElement('div')
        code_segment_btn_left_absolute_block.setAttribute('class', 'div_code_segment_left_absolute_block')
        
        let filename = code_segment_meta.getAttribute("file")
        let filename_block = null
        if(filename){
            filename_block = document.createElement('div')
            filename_block.setAttribute('class', 'filename')
            filename_block.innerHTML = `${filename}`
        }

        // 创建按钮内容 - List index
        let code_segment_btn_list_index = document.createElement('div')
        code_segment_btn_list_index.setAttribute('class', 'div_code_segment_list_index')
        if(language === 'cn'){
            code_segment_btn_list_index.innerHTML = `代码块 ${i+1}`
        } else {
            code_segment_btn_list_index.innerHTML = `List ${i+1}`
        }
        if(filename){
            code_segment_btn_list_index.append(filename_block)
        }
        code_segment_btn_list_index.setAttribute('id', `code_segment_id_${i+1}`)
        code_segment_btn_list_index.setAttribute('name', `code_segment_id_${i+1}`)
        code_segment_btn_left_absolute_block.append(code_segment_btn_list_index)
        ...
        code_segment_btn_left_absolute_block.append(code_segment_btn_language_type)

        code_segment_btn.append(code_segment_btn_content)
        code_segment_btn.append(code_segment_btn_left_absolute_block)
        ...
        code_segments[i].setAttribute("style", "margin:0px;") // for cactus-white
    ...
    let code_segments_ref = document.getElementsByTagName('coderef')
    for(let i = 0; i < code_segments_ref.length; i++){
        let selected_cs_index = code_segment_index_list[code_segments_ref[i].innerHTML]
        
        // 创建链接
        let cs_link = document.createElement('a')
        cs_link.setAttribute('href', `#code_segment_id_${selected_cs_index}`)

         // 修改 imgref 的内部内容
        if(language === 'cn'){
            cs_link.innerHTML = `代码块 ${selected_cs_index}`
        } else {
            cs_link.innerHTML = `List ${selected_cs_index}`
        }

         // 塞入原先位置
         code_segments_ref[i].innerHTML = ''
         code_segments_ref[i].append(cs_link)
    }
}
