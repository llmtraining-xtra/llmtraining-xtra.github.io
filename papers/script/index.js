async function __load_from_github() {
    ...
    let html_str = await res.text()

    console.log(html_str)

    // parse html
    const parser = new DOMParser();
    const doc = parser.parseFromString(html_str, 'application/xml');
    ...
    if(source_div === null){
        dest_div.innerHTML = 
            `<p style="color:red;font-size:12px;">[x] failed to load from the repo, check your connection with github</p>`
    } else {
